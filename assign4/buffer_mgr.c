...
RC initBufferPool(BM_BufferPool *const bm, const char *const pageFileName, const int numPages, ReplacementStrategy strategy,void *stratData){
  ...
  if (status == RC_FILE_NOT_FOUND){
    status = createPageFile(pageFileName);
    if(status != RC_OK) return status;
    status = openPageFile(pageFileName, bm->fh);
    if(status != RC_OK) return status; 
  }
...
RC shutdownBufferPool(BM_BufferPool *const bm){
  ...
  if(DEBUG) printf("INFO buffer_mgr.c - shutdownBufferPool: Page File Closed, Freeing rest of data\n");
  free(bm->fh);
